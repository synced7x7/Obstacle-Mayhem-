...
//        printf("orochi_face= %d\n", orochi_face_sprite);
    ...
    else if (hurt_c2_sprite11)
    {
        ...
        else if(halt_flag==4)
        {
            ...
            if (frameTime >= FPS / 5) /// FPS/x (increase x to give more speed in animation)
            {
                ...
                hurt_c2_frame11++; // Move to the next frame
                restrict_keypress=1;
                if (hurt_c2_frame11 >= 10 && halt_screen) /// Change this if there are more frames ///det how much frames the sprite will run
                {
                    hurt_c2_frame11 = 10; //Reset to the first frame
                }
                else if (hurt_c2_frame11 >= 10 && !halt_screen)
                {
                    printf("working\n");
                    reset_iori_frame();
                    activateSprite(5);
    ...
    else if (interaction_start_sprite12)
    {
        ...
        {
            frameTime = 0;
            if(interaction_start_frame12>0 && !halt_screen)
                interaction_start_frame12--;
            else if(interaction_start_frame12==0)
            {
                reset_iori_frame();
                activateSprite(1);
    ...
    if (kyo_health_sprite)
    {
        ...
        if (kyo_health_frameTime >= FPS / 7) /// FPS/x (increase x to give more speed in animation)
        {
            ...
            if(collide==1 && (hit_combo1_sprite7+hit_combo2_sprite8+running_hit_combo3_sprite9)>0 && q_count==0 && !kyo_block_sprite14) //&&!block
                kyo_health_frame++;
            if (iori_ground_effects_sprite2 && kyo_health_frame<=121 && !kyo_block_sprite14)
                kyo_health_frame+=2;
...
void cutscene_animation()
{
    //printf("iori next int = %d iori_next_sprite = %d\n", interaction_start_frame12, interaction_start_sprite12);

    if (kyo_health_frame >=120) /// Change this if there are more frames ///det how much frames the sprite will run
    {
        //Reset to the first frame

        printf("halt screen = %d, halt flag = %d\n", halt_screen, halt_flag);

        if(!halt_flag)//2nd part
        {
            kyo_health_frame=120;
            SDL_DestroyTexture(lvl_4_map_texture);
            halt_screen=50;
            halt_flag++;
            kyo_start=0;
            lvl4_part++;
        ...
        else if(!halt_screen && halt_flag==1)//2nd part animation
        {
            //printf("kyo_x=%d", kyo_texture_destination.x);
            activateKyoSprite(3);
            kyo_texture_destination.x+=5;
            restrict_keypress=1;

            if (kyo_texture_destination.x >= 493 && halt_flag==1)
            {
                map1=0;
                map2=1;
                reset_iori_frame();
        ...
        else if(!halt_screen && halt_flag==3)//last part animation
        {
            //printf("kyo_x=%d", kyo_texture_destination.x);

            activateKyoSprite(6);
            kyo_texture_destination.x-=15;
            restrict_keypress=1;
            printf("working\n");
            if (kyo_texture_destination.x <= texture_destination.x-180+262  && halt_flag==3)
            {
                map1=0,map2=0;
                SDL_DestroyTexture(lvl_4_map_texture2);
                printf("working2\n");
                reset_kyo_frame();
                resetKyoSprites();
                kyo_sp_sprite5=1;
