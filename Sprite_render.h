...
//        printf("iori next int = %d iori_next_sprite = %d\n", interaction_start_frame12, interaction_start_sprite12);
       // printf("kyo next int = %d kyo_next_sprite = %d\n", kyo_interaction_start_frame12, kyo_interaction_start_sprite12);
        if(stand_sprite1)
        {
            source_row = 0.5;
...
void sp_face()
{
    ...
    if(kyo_sp_back)
    {
        if(kyo_sp_back_texture_destination.x>300 || kyo_sp_back_texture_destination.x<=-300)
            kyo_sp_back_texture_destination.x-=speed;
        else if(kyo_sp_back_texture_destination.x<=300 && kyo_sp_back_texture_destination.x>-300)
        {
            kyo_sp_back_texture_destination.x-=10;
        }
    }
    if(iori_sp_back)
    {
        if(iori_sp_back_texture_destination.x<-300 || iori_sp_back_texture_destination.x>=300)
            iori_sp_back_texture_destination.x+=speed;
        else if(iori_sp_back_texture_destination.x>=-300 && iori_sp_back_texture_destination.x<300)
        {
            iori_sp_back_texture_destination.x+=10;
        }
    }


   // printf("kyo back = %d\n", kyo_sp_back_texture_destination.x);
   // printf("iori back = %d\n", iori_sp_back_texture_destination.x);


    if(iori_sp_back)
        SDL_RenderCopy(renderer, iori_sp_back_texture, NULL, &iori_sp_back_texture_destination);
    if(kyo_sp_back)
        SDL_RenderCopy(renderer, kyo_sp_back_texture, NULL, &kyo_sp_back_texture_destination);
}
