...
int fight_lag=0;
int block_range=70;
int easy_attack=10;//increase for easier attack
...
void kyo_algorithm_hard()
{
        ...
        if(collide==0 && rightmove ==0 && leftmove==1 && general_frame-memory==1)
        {
            boundary++;
        }
        ...
        if(boundary==special_prone && boundary_delay==0)
        {
            leftmove=0;
            rightmove=0;
        ...
        boundary_delay++;
        if(boundary_delay && boundary_delay<60 && boundary==special_prone)
        {
            rightmove=0;
            leftmove=0;
        }
        if(boundary_delay==60 && boundary==special_prone)
        {
             boundary_delay=0;
             boundary=0;
        }
        ...
        //code by ekanto
        if (iori_fin_effects_sprite3 && (kyo_texture_destination.x - texture_destination.x) > 100)
        {
            kyo_recognised = 1;
